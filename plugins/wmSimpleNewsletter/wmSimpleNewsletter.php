...
class WmSimpleNewsletter
{
	...
	$charset_collate = '';
        ...
        if ( self::$wpdb->get_var( "SHOW TABLES LIKE '" . $wpdb->WmNewsletterEmails . "'" ) != $wpdb->WmNewsletterEmails )
        {
            $sWmNewsletterEmails =
				...
				`nome_newsletter` VARCHAR(255) DEFAULT NULL,
    ...
    public function add_menu_admin()
    {
        ...
        $role->add_cap( "Newsletter" );
        if ( function_exists( 'add_menu_page' ) )
        {
            add_menu_page( "Newsletter", "Newsletter", null, "Newsletter" );
            add_submenu_page( "Newsletter", "Lista Newsletter", "Lista Newsletter", "Newsletter", "list-emails", array( &$this, 'open_page_admin' ) );
